...
int pay[10], nq, startrank[5], startsuit[5];
...
int *go(int runnr, VI ranks, VI &rankleft)
{
	...
	else
	{
		...
		if (ret[0] == 0)
		{
			...
			VPII cnt;
			REP(i, 5)
			if (SZ(cnt) == 0 || ranks[i] != cnt.back().second)
			{
				cnt.PB(MP(1, ranks[i]));
			}
			else
			...
			{
			case 1:
				ret[ranks[0] == 0 && ranks[1] == 9 ? bROYALSTRAIGHT : ranks[4] == ranks[0] + 4 ? bSTRAIGHT : bNONE]++;
				break;
			case 2:
				ret[cnt[1].first == 2 ? bTWOPAIR : bONEPAIR]++;
				break;
			case 3:
				ret[cnt[1].first == 2 ? bFULLHOUSE : bTHREEKIND]++;
				break;
			case 4:
				ret[bFOURKIND]++;
				break;
...
int readRank()
{
	char c;
	scanf(" %c", &c);
	return string("A23456789TJQK").find(c);
}
int readSuit()
{
	char c;
	scanf(" %c", &c);
	return string("cdhs").find(c);
}
int runnr;
void run()
{
	...
	REP(q, nq)
	{
		REP(i, 5)
		{
			startrank[i] = readRank();
			startsuit[i] = readSuit();
		}
		double ret = 0;
		REP(keep, 1 << 5)
		{
			...
			FOR(suit, -1, 4)
			{
				...
				case bNONE:
					...
					break;
				case bSTRAIGHT:
					res[aSTRAIGHT] += (suit != -1 ? -1 : +1) * cnt[i];
					if (suit != -1)
					{
						res[aSTRAIGHTFLUSH] += cnt[i];
					}
					break;
				case bROYALSTRAIGHT:
					res[aSTRAIGHT] += (suit != -1 ? -1 : +1) * cnt[i];
				...
				case bTHREEKIND:
					assert(suit == -1);
					res[aTHREEKIND] += cnt[i];
					break;
				case bFULLHOUSE:
					assert(suit == -1);
					res[aFULLHOUSE] += cnt[i];
					break;
				case bFOURKIND:
					assert(suit == -1);
					res[aFOURKIND] += cnt[i];
