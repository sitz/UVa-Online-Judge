...
char map_[20][20];
int link[20][20], conn[305][7], vis[305][305][305], beg[3], end_[3], w, h, n, num, step_fro, step_back, back_fro, back_rear, fro_fro, fro_rear;

void bfs()
{
	int fro_cnt = 1, back_cnt = 1;
	vis[beg[0]][beg[1]][beg[2]] = ++step_fro;
	vis[end_[0]][end_[1]][end_[2]] = ++step_back;
	fro_fro = back_fro = 0;
	fro_rear = back_rear = 1;
	queue_fro[0].a = beg[0], queue_fro[0].b = beg[1], queue_fro[0].c = beg[2];
	queue_back[0].a = end_[0], queue_back[0].b = end_[1], queue_back[0].c = end_[2];
			...
			for (int i = 0; pos_a != 0; ++i)
				for (int j = 0; pos_b != 0; ++j)
					for (int k = 0; pos_c != 0; ++k)
					{
						if (beg[2] != 303)
						{
							if (pos_a == queue_fro[fro_fro].b && pos_b == queue_fro[fro_fro].a)
								continue;
							if (pos_a == queue_fro[fro_fro].c && pos_c == queue_fro[fro_fro].a)
								continue;
							if (pos_b == queue_fro[fro_fro].c && pos_c == queue_fro[fro_fro].b)
								continue;
							if (pos_a == pos_b || pos_a == pos_c || pos_b == pos_c)
								continue;
						}
						else if (beg[1] != 303)
						{
						...
						if (vis[pos_a][pos_b][pos_c] > 1000)
							return;
						if (vis[pos_a][pos_b][pos_c] > 0)
							continue;
			...
			for (int i = 0; pos_A != 0; ++i)
				for (int j = 0; pos_B != 0; ++j)
					for (int k = 0; pos_C != 0; ++k)
					{
						if (beg[2] != 303)
						{
							if (pos_A == queue_back[back_fro].b && pos_B == queue_back[back_fro].a)
								continue;
							if (pos_A == queue_back[back_fro].c && pos_C == queue_back[back_fro].a)
								continue;
							if (pos_B == queue_back[back_fro].c && pos_C == queue_back[back_fro].b)
								continue;
							if (pos_A == pos_B || pos_A == pos_C || pos_B == pos_C)
								continue;
						}
...
int main()
{
	while (scanf("%d%d%d", &w, &h, &n) == 3 && n)
	{
		...
		beg[0] = beg[1] = beg[2] = end_[0] = end_[1] = end_[2] = 303;
		conn[303][0] = 303;
		for (int i = 0; i < h; ++i)
			for (int j = 0; j < w; ++j)
			{
				if (islower(map_[i][j]))
					beg[map_[i][j] - 'a'] = pos_now;
				else if (isupper(map_[i][j]))
					end_[map_[i][j] - 'A'] = pos_now;
			}
		bfs();
