...
char w[N];
int H = 0, bucket[N], lcp[N], new_bucket[N], rank_[N], sn[N];
...
struct suffix
{
	...
	bool operator<(const suffix &s) const
	{
		...
		if (bucket[z] == bucket[s.z])
		{
			return bucket[z + H] < bucket[s.z + H];
		}
...
int main()
{
	...
	for (int n; scanf("%d", &n) == 1 && n; first = false)
	{
		...
		for (int i = 0, in = 0, j = 0; i < L; ++i)
		{
			...
			{
				j++;
				if (sn[pos[j].z] != -1 && !cnt[sn[pos[j].z]]++)
				{
					++in;
				}
			}
			...
			if (sn[pos[i].z] != -1 && !--cnt[sn[pos[i].z]])
			{
				--in;
			}
		}
