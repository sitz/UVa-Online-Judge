...
inline void Set(int rt)
{
	SegTree[rt].rev ^= 1;
	if (SegTree[rt].mi != -1)
		SegTree[rt].mi = L - SegTree[rt].mi;
	if (SegTree[rt].ma != -1)
		SegTree[rt].ma = L - SegTree[rt].ma;
...
void reverse(int rt, int l, int r, int L, int R)
{
	...
	pushdown(rt);
	if (L < mid)
		reverse(lson, l, mid, L, R);
	if (R > mid)
		reverse(rson, mid, r, L, R);
	update(rt);
}
...
int query(int rt, int l, int r, int L, int R)
{
	if (R <= L)
		return -1;
	if (L <= l && R >= r)
		return SegTree[rt].ma;
	pushdown(rt);
	int ret = -1;
	if (L < mid)
		ret = tmax(ret, query(lson, l, mid, L, R));
	if (R > mid)
		ret = tmax(ret, query(rson, mid, r, L, R));
	return ret;
}
