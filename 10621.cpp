	...
	for (int cs = 1; scanf("%d", &N) == 1 && N > 0; cs++)
	{
		...
		while (!Q.empty())
		{
			...
			for (int dir1 = 0; dir1 < 5; dir1++)
			{
				...
				if ((dir1 == 4) ^ (maze[jack] == 'S'))
				{
					continue;
				}
				if (maze[ja] == '*' || maze[ja] == 's' || maze[ja] == 'h')
				{
					continue;
				}
				for (int dir2 = 0; dir2 < 5; dir2++)
				{
					...
					if ((dir2 == 4) ^ (maze[jill] == 's'))
					{
						continue;
					}
					if (maze[ji] == '*' || maze[ji] == 'S' || maze[ji] == 'H')
					{
						continue;
					}
					if (seen[ja][ji] != cs)
		...
		while (jack != H || jill != h)
		{
			int dir1 = prev_[jack][jill] / 8;
			int dir2 = prev_[jack][jill] % 8;
			if (dir1 < 4)
			{
				str[0] += delta_c[dir1];
			}
