...
// geom
// ret 1 if a > b, 0 if a == b and -1 if a < b
...
struct Point
{
	...
	_T Dot(const Point &p) const
	{
		return x * p.x + y * p.y;
	}
	_T Cross(const Point &p) const
	{
		return x * p.y - y * p.x;
	}
...
{
	if (p.x < min(b.x, e.x) || p.x > max(b.x, e.x))
	{
		return false;
	}
	if (p.y < min(b.y, e.y) || p.y > max(b.y, e.y))
	{
		return false;
	}
	return true;
...
bool SegmentIntersect(Point<_T> &p1, Point<_T> &p2, Point<_T> &p3, Point<_T> &p4)
{
	...
	if (d1 == 0 && InInterval(p3, p1, p2))
	{
		return true;
	}
	if (d2 == 0 && InInterval(p4, p1, p2))
	{
		return true;
	}
	if (d3 == 0 && InInterval(p1, p3, p4))
	{
		return true;
	}
	if (d4 == 0 && InInterval(p2, p3, p4))
	{
		return true;
	}
	return false;
...
// tarjan
int n, num[NN], low[NN], scc[NN], cnt, scccnt, s[NN], top;

void dfs(int u)
{
	num[u] = low[u] = cnt++;
	s[top++] = u;
	for (int i = nxt[u]; i != -1; i = e[i].nxt)
	{
		int v = e[i].to;
		if (num[v] == -1)
		{
...
int main()
{
	...
	while (scanf("%d", &n) == 1 && n)
	{
		...
		{
			FOR(j, 4)
			scanf("%lld %lld", &vr[i].p[j].x, &vr[i].p[j].y);
