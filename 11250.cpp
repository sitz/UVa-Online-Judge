...
private:
	...
	static void absSub(BigInt &z, const BigInt &x, const BigInt &y)
	{
		...
		assert(c == 1);
		z.size = xn;
		while (z.size > 0 && z.data[z.size - 1] == 0)
		{
			z.size--;
		}
		assert(z.size > 0);
	...
	static void absDiv(BigInt *q, BigInt &u, BigInt v)
	{
		...
		int n = v.size, d = 0;
		while (((v.data[n - 1] << d) & 0x80000000U) == 0)
		{
			d++;
		}
		u <<= d;
...
public:
	...
	BigInt(const char *s, int radix = 10)
	{
		data = NULL;
		assign(s, radix);
	}
	BigInt(const string &s, int radix = 10)
	{
		data = NULL;
		assign(s.c_str(), radix);
	}
	...
	string toString(int r = 10) const
	{
		return str(r);
	}
	int toInt() const
	{
		return sgn(size) * (int)data[0];
	}
	long long toLL() const
	{
		long long r = 0;
		for (int i = 0; i < 2 && i < abs(size); i++)
		{
			r = (r << 32) | data[i];
		}
		return size < 0 ? -r : r;
	...
	{
		return (abs(size) > (n >> 5)) ? ((data[n >> 5] >> (n & 31)) & 1) : 0;
	}
	...
	int compareTo(const BigInt &y) const
	{
		return cmp(*this, y);
	}
	int compareTo(int y) const
	{
		return cmp1s(*this, y);
	}
};
...
struct Rational
{
	...
	R operator*(const R &b) const
	{
		return R(p * b.p, q * b.q);
	}
	R operator/(const R &b) const
	{
		return R(p * b.q, q * b.p);
	}
	R operator-() const
	{
		return R(-p, q);
...
Rational<BigInt> f(long long n)
{
	typedef Rational<BigInt> R;
	R part1(0, 1);
	if (n <= 6)
	{
	...
	else
	{
		...
		for (long long k = n + 1; k <= n + 3; k++)
		{
			part1 = part1 - R(1, k);
		}
	}
	part1 = part1 / R(6, 1);
	R part2 = (R(1, n + 2) - R(1, 2)) / R(2, 1);
	return part1 + part2;
}
...
{
	long long n, m;
	for (int t = 1; scanf("%lld%lld", &n, &m) == 2; t++)
	{
		if (n == 0 && m == 0)
		{
			break;
		}
		Rational<BigInt> res = f(n) * f(m);
		string p = res.p.toString();
		string q = res.q.toString();
		printf("Case %d:\n%s\n", t,p.c_str());
