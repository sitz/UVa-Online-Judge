...
typedef struct {unsigned n, a[16];} num_t;
...
void div(num_t *q, num_t *r, num_t *x, num_t *y)
{
	...
	rshift(r, ns);
	rshift(y, ns);
	while (r->n > 0 && r->a[r->n - 1] == 0)
	{
		r->n--;
	}
	while (q->n > 0 && q->a[q->n - 1] == 0)
	{
		q->n--;
	}
}
