...
struct Node
{
	...
	Node(char *S, Node *A = NULL, Node *B = NULL, Node *C = NULL) : what(S), a(A), b(B), c(C) {}
};
...
struct Parser
{
	...
	int eq(char *word)
	{
		if (memcmp(p, word, strlen(word)) != 0)
		{
			return 0;
		}
		p += strlen(word);
	...
	Node *cond()
	{
		...
		if (eq("AND"))
		{
			return new Node("AND", z, cond());
		}
		if (eq("OR"))
		{
			return new Node("OR", z, cond());
		}
		return z;
	}
	...
	Node *stmt()
	{
		if (eq("TRADE"))
		{
			return new Node("TRADE");
		}
		if (eq("CHEAT"))
		{
			return new Node("CHEAT");
		}
		if (!eq("IF"))
		{
			assert(0 == 1);
