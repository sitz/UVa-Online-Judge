...
long long count(long long r1, long long r2, long long c1, long long c2)
{
	...
	if (r2 - 1 <= c2 - 2)
	{
		if (c1 - 1 >= r1)
		{
			return f(r2 - c1 + 1) + (c1 - r1 - 1) * (c2 - c1) + (r2 - c1 + 1) * (c2 - r2 - 1);
		}
...
int main()
{
	...
	for (int tc = 1; tc <= T; ++tc)
	{
		...
		for (int i = 0; i < M; ++i)
		{
			...
			int c1 = param[1] - 1;
			int c2 = c1 + param[2] - 1;
			if (param.size() == 4 && param[3] % 2 == 1)
			{
				add(r1, r1 + param[2] - 1, c1, c2);
			}
			if (param.size() == 5 && param[4] % 2 == 1)
			{
				add(r1, r1 + param[3] - 1, c1, c2);
			}
		}
