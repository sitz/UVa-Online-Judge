...
{
	double res = (p1 - p0).dot(q - p0) / (p1 - p0).dist2();
	if (res < 0.0)
	{
		res = 0.0;
	}
	double t = min(res, 1.0);
	return sqrt((p0 * (1 - t) - p1 * -t - q).dist2());
}

double d(P p0, P p1, P q0, P q1)
{
	return min(min(min(d(p0, p1, q0), d(p0, p1, q1)), d(q0, q1, p0)), d(q0, q1, p1));
}
...
void solve()
{
	...
	for (int i1 = 0, j1 = N1 - 1; i1 < N1; j1 = i1++)
		...
		{
			res = min(res, d(P1[i1], P1[j1], P2[i2], P2[j2]));
		}
	printf("%.9lf\n", res / 2);
}
