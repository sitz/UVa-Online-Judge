...
long long rec(int i, int j, int flag)
{
	...
	if (flag == 0)
	{
		res = MIN(res, rec(i, j, 1) + extraCost);
	}
	res = MIN(res, rec(i + 1, j, 0));
	if (flag == 1 && boxes[j].m <= boxTypes[i].m)
	{
		res = MIN(res, boxes[j].count * (long long)boxTypes[i].price + rec(i, j + 1, 1));
	}
	return res;
}
