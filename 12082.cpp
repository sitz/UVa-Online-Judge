...
bool is_stuck(int x, int y)
{
	...
	FOR(dir, 6)
	{
		...
		if (!g[nx][ny].e && ++ccnt[map_[g[nx][ny].c[(dir + 3) % 6]]] == 3)
		{
			return true;
		}
	}
...
bool forced_impossible(int x, int y)
{
	...
	if (cnt == 3)// its forced
	{
		...
		FOR(dir, 6)
		{
			...
			{
				if (++ccnt[map_[g[nx][ny].c[(dir + 3) % 6]]] == 3)
				{
					return true;
				}
			}
