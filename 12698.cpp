...
bool edge_to_edge(const Point &L, const Point &R, const Point &L2, const Point &R2)
{
	...
	{
		if (L != L2)
			return point_to_edge(L, L2, R2);
	}
	if (L2.x >= L.x && L2.x <= R.x)
	{
		if (L2 != L)
			return !point_to_edge(L2, L, R);
	}
	if (R.x >= L2.x && R.x <= R2.x)
	{
		if (R != R2)
			return point_to_edge(R, L2, R2);
	}
	if (R2.x >= L.x && R2.x <= R.x)
	{
		if (R != R2)
			return !point_to_edge(R2, L, R);
	}
	return 0;
}
...
struct Compare
{
	bool operator()(int x, int y)
	{
		...
		int x_pid = x & 3;
		int x_pid2 = x_pid + 1;
		if (tri[x_id][x_pid].x > tri[x_id][x_pid2].x)
			swap(x_pid, x_pid2);
		int y_id = y >> 2;
		int y_pid = y & 3;
		int y_pid2 = y_pid + 1;
		if (tri[y_id][y_pid].x > tri[y_id][y_pid2].x)
			swap(y_pid, y_pid2);

		if (tri[x_id][x_pid] == tri[x_id][x_pid2])
			return point_to_edge(tri[x_id][x_pid], tri[y_id][y_pid], tri[y_id][y_pid2]);
		if (tri[y_id][y_pid] == tri[y_id][y_pid2])
			return !point_to_edge(tri[y_id][y_pid], tri[x_id][x_pid], tri[x_id][x_pid2]);
...
int main()
{
	...
	while (T--)
	{
		...
		while (N--)
		{
			...
			{
				REP(i, 3)
				scanf("%d%d", &tri[tn][i].x, &tri[tn][i].y);
				if (d > 0)
					REP(i, 3)
					{
						tri[tn][i].x += tri[d][0].x;
						tri[tn][i].y += tri[d][0].y;
					}
				tri[tn][3] = tri[tn][0];
				now_id = tn;
				REP(i, 3)
