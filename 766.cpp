...
struct Frac
{
	...
	inline void reduce()
	{
		...
		BigInt g = __gcd(num < 0 ? -num : num, den);
		num /= g;
		den /= g;
	}
	...
	Frac(const BigInt &x, const BigInt &y) : num(x), den(y)
	{
		reduce();
	}
...
void gauss()
{
	for (int i = 0; i < N; i++)
	{
		...
		{
			int k;
			for (k = i + 1; k < N && mat[k][i].isZero(); k++)
				;
			assert(0 <= k && k < N && !mat[k][i].isZero());
			for (int j = i; j <= N; j++)
			{
...
int main()
{
	...
	for (int cs = 1; cs <= T; cs++)
	{
		...
		for (int i = 0; i < N; i++)
		{
			BigInt t = mat[i][N].num * M / mat[i][N].den;
			s += " " + t.str();
		}
		s += " 0";
		answer[K] = new char[1000];
		strcpy(answer[K], s.c_str());
		printf("%s\n", answer[K]);
	}
}
