...
struct edge {int to, w;} adj[N][N];

bool sccan[N], cs[N], onstk[N], onq[N], nothing;
int d[N], deg[N], idx[N], __idx, low[N], q[N], scc[N], stk[N], scc_count, top;
...
int main()
{
	...
	for (int cnum = 0; cnum++ < t;)
	{
		...
		for (int i = 0; i < n; ++i)
		{
			sccan[i] = cs[i] = deg[i] = 0, scc[i] = idx[i] = -1;
		}
		for (int i = 0; i < m; ++i)
		{
			readint(&a);
		...
		for (int i = 0; i < n; ++i)
		{
			...
			if (cs[i] | (scc[i] != -1 && sccan[scc[i]]))
			{
				nothing = 0, printf(" %d", i);
			}
		}
		if (nothing)
		{
