...
/**
	require from 0 to n-1's longest palindromic path and
	output minimum lexicographical palindrome

	since walk limited to i->j (i<j) only, can use DP to solve:
	dp[start][end] = max(dp[p][q]+2); if start->p ... q->end when p <= q

	record min value from which later can backtrack
