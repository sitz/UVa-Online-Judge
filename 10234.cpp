...
bool xcmp(int s, int t)
{
	...
	while (tolower(data[s]) == tolower(data[t]))
	{
		s++, t++;
	}
...
{
	return s != t && strcmp(data + s, data + t) < 0;
}
...
int main()
{
	while (gets(data))
	{
		if (gets(buf) == NULL || sscanf(buf, "%d", &T) != 1)
		{
			break;
		}
		N = strlen(data);
		...
		{
			assert(1 <= m && m <= N);
			for (int i = 0; i < strlen(data + ans[m]); i++)
			{
				data[ans[m] + i] = tolower(data[ans[m] + i]);
			}
