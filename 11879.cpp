...
#define MAXDIGITS 1100 /* maximum length bignum */
...
int digit_shift_div10(bignum *n)
{
	int i;
	int result;
	result = n->digits[0];
...
int main()
{
	string input;
	int remainder;
	bignum n, tmp, tmp_result, five, zero;
	int_to_bignum(5, &five);
	...
	while (getline(cin, input))
	{
		...
		string_to_bignum(input, &n);
		while (n.lastdigit > 8)
		{
			remainder = digit_shift_div10(&n);
			remainder *= 5;
			int_to_bignum(remainder, &tmp);
			subtract_bignum(&n, &tmp, &tmp_result);
			add_bignum(&tmp_result, &zero, &n);
		}
		int result = bignum_to_int(&n);
		if (result % 17 == 0)
		{
			cout << 1 << endl;
		}
