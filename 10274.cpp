...
// //
Map que[MAXN];
int head[MAXN], nxt[MAXN], dist[MAXN];

void inInit()
{
	memset(head, 0, sizeof(head));
	memset(que, 0, sizeof(que));
	memcpy(que[1].state, start.state, sizeof(start.state));
	dist[1] = 0;
}

inline int hash_(Map &cur)
{
	int seed = 131, v = 0;
	for (int i = 0; i < r; ++i)
	{
...
int main()
{
	...
	while (cas--)
	{
		...
		{
			gets(start.state[i]);
			int len = strlen(start.state[i]);
			for (int j = 0; j < len; j++)
				if (start.state[i][j] > '0' && start.state[i][j] < '4')
				{
					sumGem++;
				}
		}
