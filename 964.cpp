...
bool push(const Line &l)
{
	if (l.argVar >= 0)
	{
		if (sym[l.argVar].written != 0)
		{
			return push(sym[l.argVar].value);
		}
	}
	else
	{
...
int main()
{
	...
	for (;;)
	{
		...
		for (;;)
		{
			...
			for (int m = strlen(s); m > 0 && isspace(s[m - 1]);)
			{
				s[--m] = 0;
			}
			assert(isalpha(s[0]));
