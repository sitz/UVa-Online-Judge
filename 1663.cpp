		...
		for (int i = 0; i < D.size(); i++)
		{
			if (color[D[i]] == 0)
			{
				for (int j = 0; j < m; j++)
				{
					if (used[D[i] ^ (1 << j)])
					{
						g.add_edge(D[i], D[i] ^ (1 << j), 1);
					}
				}
				g.add_edge(src, D[i], 1);
			}
			else
			{
				g.add_edge(D[i], sink, 1);
			}
		}

		int ret = (int)D.size() - g.maxflow(src, sink);
		printf("%d\n", ret);
	}
	return 0;
}
