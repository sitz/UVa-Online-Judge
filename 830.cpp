...
int identifyAnimal(int x, int y)
{
	...
	while (rx + 1 < n && g[rx + 1][ry] == v)
		rx++;
	while (ry + 1 < m && g[rx][ry + 1] == v)
		ry++;
	lx = rx, ly = ry;
	while (lx - 1 >= 0 && g[lx - 1][ly] == v)
		lx--;
	while (ly - 1 >= 0 && g[lx][ly - 1] == v)
		ly--;

	int isShark = 0, isDolphin = 0;
	for (int i = ly; i <= ry; i++)
	{
		if (lx - 1 >= 0 && g[lx - 1][i] == v)
			isShark = 1;
		if (rx + 1 < n && g[rx + 1][i] == v)
			isShark = 1;
	}
	for (int i = lx; i <= rx; i++)
	{
		if (ly - 1 >= 0 && g[i][ly - 1] == v)
			isShark = 1;
		if (ry + 1 < m && g[i][ry + 1] == v)
			isShark = 1;
	}
	w = ry - ly + 1, h = rx - lx + 1;
	if (w > h)
