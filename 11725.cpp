		...
		for (int m = 0; m < M; ++m)
		{
			...
			for (int ii = 0; ii < C; ++ii)
			{
				...
				for (int j = 0; j < C; ++j)
				{
					...
					{
						if (n > 0 && forbidden[m][n - 1])
						{
							continue;
						}
						if (m > 0 && forbidden[m - 1][n])
						{
							continue;
						}
						int mask = 0x03 << (n * 2);
						if (n > 0 && ((i & mask) >> 2) == (i & (mask >> 2)))
						{
							good = false;
						}
						else if (m > 0 && (i & mask) == (j & mask))
						{
							good = false;
						}
					}
...
#endif
		cerr << liner << "EXPECTED" << liner << endl
				 << output;
		cerr << liner << "-ACTUAL-" << liner << endl
				 << os.str()
