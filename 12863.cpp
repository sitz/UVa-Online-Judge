...
int V[MAXN][MAXN], R[MAXN][MAXN], C[MAXN][MAXN], TX, TY;

struct Node
{
	int r, c, v, h;
	Node(int a = 0, int b = 0, int d = 0, int e = 0) : r(a), c(b), v(d), h(e) {}
...
{ // 2D binary indexed tree
	...
	void update(int lx, int rx, int ly, int ry, int val, int tot)
	{ // {val: update cost, tot: #unvisited point in area.}
		if (tot == -1)
		{
