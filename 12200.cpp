...
//p[m][n][0] = prob of winning A if A starts, A with m points and B with n.
//p[m][n][1] = prob of winning B if B starts, A with m points and B with n.
...
int main()
{
	...
	for (int m = 1; m <= MAXN; m++)
		for (int n = 1; n <= MAXN; n++)
		{
			...
			//solving 4 implicit equations to maximize p[m][n][1]
			p[m][n][1] = -1e9;
			for (int i = 0; i < 4; i++)
				if (pr1[i] > -0.5)
				{
