...
double solve()
{
	...
	else
	{
		for (i = 0; i < n; i++)
		{
			...
			if (fabs(dist(E, F) + dist(fpt(L), F) - dist(E, fpt(L))) < EPS)
			{
				continue;
			}
			if (F.x < (P[i].x - EPS) && F.x < (P[i + 1].x - EPS))
			{
				continue;
			}
			if (F.x > (P[i].x + EPS) && F.x > (P[i + 1].x + EPS))
			...
			if (F.y < (P[i].y - EPS) && F.y < (P[i + 1].y - EPS))
			{
				continue;
			}
			if (F.y > (P[i].y + EPS) && F.y > (P[i + 1].y + EPS))
