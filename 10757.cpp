...
struct Table
{
	...
	int columnByName(const char *s) const
	{
		for (int i = 0; i < ncols; i++)
			if (strcasecmp(col[i].name.c_str(), s) == 0)
			{
				return i;
			}
		assert(false);
...
struct Where
{
	...
	Where(int o, const char *s, Where *l = NULL, Where *r = NULL) : op(o), value(s), left(l), right(r), tmp(-1) {}
...
class WhereEvaluator
{
	...
	bool eval(Where *w)
	{
		...
		if (w->op == '&')
		{
			return eval(w->left) && eval(w->right);
		}
		if (w->op == '|')
		{
			return eval(w->left) || eval(w->right);
...
Table *parseFrom()
{
	...
	for (;;)
	{
		if (tok != 'a' || strcasecmp(tok_s, "INNER") != 0)
		{
			return A;
		}
		skip("INNER");
...
{
	if (tok != 'a' || strcasecmp(tok_s, "WHERE") != 0)
	{
		return NULL;
	}
	skip("WHERE");
