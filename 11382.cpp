...
int main()
{
	...
	for (int h = 0; h < casos; h++)
	{
		...
		for (i = 0; i < n; i++)
		{
			intervalo.push_back(inter(seg[i]));
		}
		...
		for (i = 0; i < intervalo.size(); i++)
			if (intervalo[i].first > EPS + intervalo[i].second)
			{
		...
		else
		{
			...
			b = (1.0 / 180.0) * PI;
			e = PI;
			if (cantidad(e, intervalo) > K)
			{
				res = -1.0;
			}
			else if (cantidad(b, intervalo) <= K)
			{
				res = b;
			}
			else
