...
struct STATE
{
	char x, y, r;
	int s;
	STATE(char _x = 0, char _y = 0, char _r = 0, int _s = 0) : x(_x), y(_y), r(_r), s(_s) {}
};
...
int BFS()
{
	...
	while (!q.empty())
	{
		...
		for (char i = 0; i < 3; ++i)
		{
			...
			{
				if (!connected[c_x][c_y][(dir + new_rot) % 4])
				{
					continue;
				}
				...
				for (char j = 0; j < 4; ++j)
				{
					if (abs(rotn[j]) > spare_r)
					{
						continue;
					}
					char new_rot2 = (rotn[j] + 4) % 4;
					if (!connected[nx][ny][(dir + new_rot2 + 2) % 4])
					{
						continue;
					}
	...
	while (scanf("%d%d", &R, &C) && R)
	{
		...
		for (char i = 0; i < R; ++i)
		{
			for (char j = 0; j < C; ++j)
			{
				...
				for (char *L = conns; *L; L++)
				{
					connected[i][j][map_[*L]] = 1;
				}
			}
		}
		printf("%d\n", BFS());
