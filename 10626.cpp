		...
		for (int C = 0; C <= cokes; C++)
		{
			...
			for (int fives = 0; fives <= 150; fives++)
				for (int tens = 0; tens <= N[2]; tens++)
				{
					...
					if (tens >= 1)
					{
						res = MIN(res, 1 + dp[C - 1][fives][tens - 1]);
					}
					if (tens >= 1 && ones >= 3)
					{
						res = MIN(res, 4 + dp[C - 1][fives + 1][tens - 1]);
					}
					if (fives >= 1 && ones >= 3)
					{
						res = MIN(res, 4 + dp[C - 1][fives - 1][tens]);
					}
					if (fives >= 2)
					{
						res = MIN(res, 2 + dp[C - 1][fives - 2][tens]);
					}
					if (ones >= 8)
					{
						res = MIN(res, 8 + dp[C - 1][fives][tens]);
					}
				}
		}
		printf("%d\n", dp[cokes][N[1]][N[2]]);
