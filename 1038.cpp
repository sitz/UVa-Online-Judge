...
double uy, dy, lx, rx, ratio_, px1, py1, px2, py2, ax1, ay1, ax2, ay2, px[MAXN * MAXN], py[MAXN * MAXN];
int n, m;
...
bool cro(segment a, segment b)
{
	...
	if (sgn(b.y1 - a.y1) >= 0 || sgn(a.y1 - b.y2) >= 0)
	{
		return false;
	}
	if (sgn(a.x1 - b.x1) >= 0 || sgn(b.x1 - a.x2) >= 0)
	{
		return false;
	}
	return true;
...
bool check()
{
	...
	l = move(lx, px1, dex, ratio_);
	r = move(rx, px1, dex, ratio_);
	u = move(uy, py1, dey, ratio_);
	d = move(dy, py1, dey, ratio_);
	int cnt = 0;
	for (int a = 1; a <= m; a++)
	{
		if (seg2[a].dir == 0)
	...
	for (int a = 1; a <= n; a++)
	{
		seg3[a].x1 = move(seg1[a].x1, px1, dex, ratio_);
		seg3[a].x2 = move(seg1[a].x2, px1, dex, ratio_);
		seg3[a].y1 = move(seg1[a].y1, py1, dey, ratio_);
		seg3[a].y2 = move(seg1[a].y2, py1, dey, ratio_);
		seg3[a].dir = seg1[a].dir;
		if (sgn(fabs(seg3[a].x1 - seg3[a].x2) + fabs(seg3[a].y1 - seg3[a].y2) - 0.05) < 0)
		{
			return false;
		}
	}
		...
		for (int a = 1; a <= n; a++)
		{
			...
			for (int b = a + 1; b <= n; b++)
			{
				if (seg1[a].dir != seg1[b].dir && cro(seg1[a], seg1[b]))
				{
					...
					if (seg1[a].dir)
					{
						px1 = seg1[a].x1, py1 = seg1[b].y1;
					}
					else
					{
						px1 = seg1[b].x1, py1 = seg1[a].y1;
					}
					break;
				}
			}
		...
		for (int a = 1; a <= m; a++)
		{
			for (int b = a + 1; b <= m; b++)
			{
				if (seg2[a].dir != seg2[b].dir && cro(seg2[a], seg2[b]))
				{
					...
					if (seg2[a].dir)
					{
						px[cnt] = seg2[a].x1, py[cnt] = seg2[b].y1;
					}
					else
					{
						px[cnt] = seg2[b].x1, py[cnt] = seg2[a].y1;
					}
				}
			}
		}
