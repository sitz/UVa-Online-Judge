	...
	while (scanf("%lf %lf %lf %lf %lf %lf", &D, &L, &xs, &ys, &xt, &yt) == 6)
	{
		...
		for (int r = row; r >= 1; r--)
		{
			for (int c = col; c >= 1; c--)
			{
				...
				else
				{
					...
					{
						dp[r][c] = MIN(dp[r][c], Hypot(D - 2 * L, (i + 1) * (D - 2 * L)) + dp[r + i + 1][c + 1]);
					}
					for (int i = 0; min(row - r, col - (c + i)) == d; i++)
					{
						dp[r][c] = MIN(dp[r][c], Hypot(D - 2 * L, (i + 1) * (D - 2 * L)) + dp[r + 1][c + i + 1]);
					}
				}
			}
		}
		...
		for (int r = 0; r < row; r++)
		{
			for (int c = 0; (r == 0 || c == 0) && c < col; c++)
			{
				double y1 = r * D + D - L;
				double x1 = c * D + D - L;
				walk = MIN(walk, Hypot(fabs(xs - x1) - c * 2 * L, fabs(ys - y1) - r * 2 * L) + dp[r + 1][c + 1]);
			}
		}
		if (walk < EPS)
		{
