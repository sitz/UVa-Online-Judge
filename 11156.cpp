...
double match(int mask)
{
	...
	for (int b = a + 1; b < nodd; b++)
	{
		if ((mask & (1 << b)) != 0)
		{
			res = MIN(res, fw[odd[a]][odd[b]] + match(mask ^ (1 << a) ^ (1 << b)));
		}
	}
	return res;
}
	...
	for (int x = 1; x <= 5; x++)
	{
		for (int y = 1; y <= 5; y++)
		{
			...
			for (int k = 0; k < nseg; k++)
			{
				...
				{
					if (!on(p, segA[k], segB[k]))
					{
						continue;
					}
					if (!on(p, segA[m], segB[m]))
					{
						continue;
					}
					if (on(segA[k], segA[m], segB[m]))
					{
						continue;
					}
					if (on(segB[k], segA[m], segB[m]))
					{
						continue;
					}
					if (on(segA[m], segA[k], segB[k]))
					{
						continue;
					}
					if (on(segB[m], segA[k], segB[k]))
					{
						continue;
					}
					add = true;
	...
	for (int cs = 1; cs <= T; cs++)
	{
		...
		if (nodd > 0)
		{
			...
			for (int i = 0; i < nodd; i++)
				...
				{
					t = MIN(t, match(((1 << nodd) - 1) ^ (1 << i) ^ (1 << j)));
				}
			ans += t;
		}
		printf("Case %d: %.2f\n", cs, ans);
