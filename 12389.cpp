...
pair<int, int> pts[N];
int sorty[NLOG][N];
int aux1[NLOG][N], aux2[NLOG][N], root1[N], root2[N], root3[N], root4[N], n, q;
...
void build_tree(int dpt, int lft, int rgt, bool leftBranch = false)
{
	...
	if (dpt > 0)
	{
		...
		if (!leftBranch)
		{
			...
			{
				int idx = sorty[dpt][i];
				if (pts[idx].first - pts[idx].second < val1)
				{
					aux1[dpt][i] = idx;
				}
				else
				{
					aux1[dpt][i] = aux1[dpt][i - 1];
				}
			...
			{
				int idx = sorty[dpt][i];
				if (pts[idx].first + pts[idx].second < val2)
				{
					aux2[dpt][i] = idx;
				}
				else
				{
					aux2[dpt][i] = aux2[dpt][i + 1];
				}
	...
	else
	{
		...
		{
			int idx = sorty[dpt][i];
			if (-pts[idx].first - pts[idx].second < val3)
			{
				root3[i] = idx;
			}
			else
		...
		{
			int idx = sorty[dpt][i];
			if (-pts[idx].first + pts[idx].second < val4)
			{
				root4[i] = idx;
			}
			else
...
{
	if (pts[arr[lft]].second >= val)
	{
		return -1;
	}
	if (pts[arr[rgt]].second < val)
	{
		return rgt;
	}
	while (rgt - lft > 1)
